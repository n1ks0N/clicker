...
const Clicks = () => {
  ...
  const [sumTime, setSumTime] = useState(false) // считает время, проводимое на ссылках из заданий
  ...
  window.onfocus = () => {
    if (observer) {
      setSumTime(prev => prev + (Date.now() - observer) / 1000)
      setObserver(false)
    }
  };
  const clickDone = ({ id, href }) => {
    setObserver(Date.now())
  ...
  useEffect(() => {
    if (completeUrls.length - 1 === Number(category) && sumTime >= 15) {
      setSumTime(false)
      tasksDB
        .doc(`${category}`)
        .get()
        ...
        .then(() => {
          fb.firestore().collection('users').doc(`${mail}`).get().then((doc) => {
            if (doc.exists) {
              docRef.set(
                {
    ...
    <div>
      ...
      <div>
        {Object.values(tasks)
          ...
          .map(
            (data, i) =>
              !!(data.total_clicks > data.spent_clicks && data.total_clicks - data.spent_clicks >= category) && (
                <div key={i}>
                  <h3>{data.name}</h3>
